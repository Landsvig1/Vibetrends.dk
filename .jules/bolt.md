...
## 2026-07-20 - Un-memoized contextual handlers (e.g. LanguageProvider's `t` function) breaking React.memo
**Learning:** Re-rendering during search typing on a low-frequency feed (like `BlogList`) cannot be easily eliminated by wrapping card components in `React.memo` and parent handlers in `useCallback` if their dependency arrays contain props/functions (like `t` from `LanguageProvider`) that are recreated on every render of their provider. Here, because `t` is a plain arrow function recreated dynamically, `handleDeletePost` gets a new identity on every render regardless of `useCallback`, breaking the shallow-prop equality check on `<BlogPostCard />` and rendering the memoization completely ineffective.
**Action:** Before introducing `React.memo` and `useCallback` dependencies, trace the referential stability of all dependency inputs. If a context-derived value is referentially unstable, `React.memo` will silently fail. Furthermore, avoid applying identical structural extraction + memoization patterns reflexively across every list explorer; case-by-case evaluation should favor lightweight JSX extraction over complex memoization primitives for low-frequency content spaces (e.g. a small community blog).
