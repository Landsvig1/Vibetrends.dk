...
## 2026-08-02 - Flat branch conditionals and type checks in relative-time formatter
**Learning:** The relative-time formatter `timeAgo` was previously looping through a configuration array `UNITS` on every call. Across high-frequency render-lists like the forum explorer or message lists, this created unnecessary allocation overhead and loop iterations. Furthermore, wrapping numbers/timestamps eagerly in `new Date()` before checking if we already have a timestamp was redundant.
**Action:** Optimize `timeAgo` to directly verify parameter types (retaining raw numbers/timestamps immediately) and leverage a flat set of linear conditional branches (`if/else`) for different time thresholds, bypassing dynamic loop iterations and structural config allocations completely.
