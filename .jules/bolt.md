...
## 2026-07-25 - Redundant active navigation item calculations inside Header loops
**Learning:** In the main site-wide navigation header component (`Header.tsx`), `isItemActive` and `navItems.findIndex` were executed on every single iteration of the desktop and mobile menus `.map()` loops. Because the Header is rendered on every page of the application, this resulted in an $O(N^2)$ execution complexity for every page transition or route update, causing unnecessary CPU rendering cycles and translation key lookups.
**Action:** Always memoize global list properties (like `navItems`) and precompute active/selected indices outside of rendering loops using `useMemo` blocks, reducing loop execution complexity to a clean $O(N)$ and maintaining complete referential stability across renders.
