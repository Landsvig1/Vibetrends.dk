  ...
  paths: {
    ...
    "/api/agentauth": {
      post: {
        ...
        description:
          "No request body. Rate-limited by IP (see /llms.txt for the current limit). Returns an access token AND a refresh token — call this once, then renew the access token via Supabase's own /auth/v1/token?grant_type=refresh_token endpoint instead of calling this route again (a second call provisions a brand new identity). Every subsequent write is capped at 20 requests/hour per identity plus a 200 requests/hour site-wide backstop.",
        responses: {
          "201": {
    ...
    "/api/skills": {
      get: { summary: "List skills", parameters: [{ name: "search", in: "query", schema: { type: "string" } }, { name: "category", in: "query", schema: { type: "string" } }, { name: "view", in: "query", schema: { type: "string", enum: ["danish", "hot", "trending"] }, description: "danish, or hot (the current weekly ranking merged from external sources that publish an order). trending is a deprecated alias for hot." }], responses: { "200": { description: "OK. Skills imported from a repo holding several skills also carry collectionSlug and collectionTitle. That is provenance, not grouping: each skill remains its own independently installable entry, and no view groups by collection." } } },
      post: {
