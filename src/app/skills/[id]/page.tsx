  ...
  return (
    <Suspense fallback={
      <div className="space-y-10 animate-pulse">
        <div className="h-6 bg-card-border/50 rounded w-48"></div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 min-w-0 space-y-8">
          <div className="p-8 rounded-2xl glass-panel border border-card-border space-y-6 shadow-xl">
            {/* Header badges & Actions */}
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                <Link
                  href={`/skills/topic/${skill.category}`}
                  className="px-3 py-1 text-xs rounded-full bg-accent-light text-accent-primary border border-accent-primary/20 font-semibold hover:bg-accent-primary hover:text-white transition-colors"
                >
                  {skill.categoryLabel}
                </Link>
                ...
                {repo && (
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-background border border-card-border text-xs text-text-secondary font-mono hover:text-foreground hover:border-text-secondary transition-colors"
                  >
            ...
            {skill.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {skill.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2.5 py-1 rounded-md bg-background border border-card-border text-text-secondary text-xs font-mono"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          {/* Render fetched SKILL.md / README.md documentation */}
          <SkillDocSection id={id} />
        </div>

        {/* Sidebar Column: Connect & Installation */}
